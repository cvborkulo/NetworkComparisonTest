NCT <- function(data1, data2, gamma, it, binary.data=FALSE, paired=FALSE, weighted=TRUE, AND=TRUE, signlevel=.05, progressbar=TRUE, ...){ 
  ...
  # signlevel: default is .05. Is used for testing significance of difference in edge weights
  ...
  if(binary.data==FALSE) 
    ...
    edges.pval <- matrix(NA,nvars,nvars)
    # real data
    ...
    edges.pval[upper.tri(edges.pval,diag=FALSE)] <- edges.pvaltemp
    rownames(edges.pval) <- colnames(edges.pval) <- colnames(data1)
    el.pvals <- melt(edges.pval, na.rm=TRUE, value.name = 'p-value')
